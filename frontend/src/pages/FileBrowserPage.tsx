        ...
        {!loading && files.length > 0 && (
          <div className="file-tree-list">
            {files.map((f) => {
              ...
              if (f.isDir) {
                return (
                  <div key={f.path} className="file-folder-divider">
                    <span className="folder-divider-label">
                      <span className="folder-icon-small">📁</span>
                      {f.path}
                    </span>
                    <div className="folder-divider-line" />
                  </div>
                );
              }
              // 文件：渲染成卡片
              return (
                <div
                  key={f.path}
                  className={`file-tree-item file ${selectedFile === f.path ? 'active' : ''}`}
                  onClick={() => handleSelectFile(f.path)}
                >
                  <div className="file-item-icon">📄</div>
                  <div className="file-item-info">
                    <div className="file-item-name">{f.path}</div>
                  </div>

...
def filter_orphan_tool_messages(messages: list[BaseMessage]) -> list[BaseMessage]:
    ...
    while i < n:
        ...
        # 1) ToolMessage 不能独立存在：若不是紧跟在合法 tool_calls 响应链中，跳过
        if isinstance(msg, ToolMessage):
            i += 1
            continue

        # 2) 对包含 tool_calls 的 AIMessage，要求后续紧跟完整的 ToolMessage 响应链
        if isinstance(msg, AIMessage) and getattr(msg, "tool_calls", []):
            ...
            # 没有可用 id 时，直接跳过该条，避免留下不完整 tool_call 结构
            if not expected_ids:
                i += 1
                continue
            ...
            # OpenAI 兼容接口要求：tool 响应应紧随 tool_calls 消息
            while j < n and isinstance(messages[j], ToolMessage):
                tm = messages[j]
                if tm.tool_call_id in expected_ids and tm.tool_call_id not in seen_ids:
                    tool_msgs.append(tm)
...
"""
    ...
    # 避免 reviewer 上下文以孤立 ToolMessage 开头导致 OpenAI 兼容接口 400
    recent = filter_orphan_tool_messages(recent)
    recent = compress_tool_messages(recent)
...
"""
    ...
    keep_count = keep_turns * 3
    recent = messages[-keep_count:] if len(messages) > keep_count else messages
    # OpenAI 兼容接口要求 ToolMessage 必须紧跟 tool_calls 响应，先过滤孤立工具消息
    recent = filter_orphan_tool_messages(recent)
    # 同时压缩过长工具输出，避免 Reviewer 上下文过载
    recent = compress_tool_messages(recent)
    history_slot = ContextSlot("recent_history", recent, priority=2)

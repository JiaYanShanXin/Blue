...
def _run_in_container(
    ...
    try:
        ...
        try:
            ...
            print(f"[Sandbox] 获取容器日志失败，使用空日志继续: {log_error}")
            logs = b""
        ...
        try:
            ...
            print(f"[Sandbox] 容器停止失败: {stop_err}")
...
def _run_test_files_with_unittest(test_files: list[str]) -> tuple[bool, str]:
    ...
    prefix = 'python -m unittest discover -s . -p "test_*.py" -v'
    if needs_pytest:
        # 尝试用 pip 安装 pytest（容器内自动执行）
        prefix = f"pip install pytest -q && {prefix}"

    success, output = _run_in_container(prefix)
    rel_list = ", ".join(os.path.relpath(tf, WORKSPACE_DIR) for tf in test_files)
    if success:
        results.append(f"[PASS] discover ({rel_list})")
    else:
        all_passed = False
        results.append(f"[FAIL] discover ({rel_list})\n{output}")
...
def sandbox_node(state: AgentState):
    ...
    # active_files 里通常是相对于 workspace 的路径（例如 src/models/device.py）
    rel_target_file = target_file
    file_path = os.path.join(WORKSPACE_DIR, rel_target_file)
    if not os.path.exists(file_path):
        return {
            "error_trace": f"执行失败：文件未找到 {rel_target_file}",
            "retry_count": retry_count + 1
        }

    # 统一为 POSIX 风格路径，避免容器 shell 中反斜杠转义问题
    safe_rel_path = rel_target_file.replace(os.sep, "/")
    run_command = f"python -B {safe_rel_path}"
    ...
    if success:
        print(f"[Sandbox] {safe_rel_path} 运行成功！(Docker 隔离环境)")
        return {"error_trace": ""}
    else:
        error_output = output
        if len(error_output) > 1500:

...
app = FastAPI(
    title="OurAI API",
    description="OurAI 智能体框架的后端 API 服务",
    version="2.0.0",
)
...
async def _run_workflow_async(thread_id: str, initial_messages: list, max_retries: int, max_coder_steps: int):
    ...
    # 构建初始状态，包含用户消息和配置参数
    initial_state = {
        "messages": [msg if isinstance(msg, BaseMessage) else HumanMessage(content=msg) for msg in initial_messages],
